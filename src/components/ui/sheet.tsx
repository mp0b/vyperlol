...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-card shadow-2xl border-border transition ease-in-out data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-200 data-[state=open]:duration-300",
  {
    variants: {
      side: {
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        className={cn(sheetVariants({ side }), "flex flex-col overflow-y-auto p-6", className)}
        {...props}
      >
        {children}
        {showClose && (
          <SheetPrimitive.Close className="absolute right-4 top-4 rounded-md opacity-60 transition-opacity hover:opacity-100">
            <X className="size-4" />
